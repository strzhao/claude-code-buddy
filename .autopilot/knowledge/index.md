...
## 2026-05-31 translate 插件优化 新增条目
- [2026-05-31] LLM 输出内嵌 `<action:speak/copy>` 标签 + 前端 MarkdownActionParser 渲染为 SwiftUI Button（方案 B：意图与交互同源，避前端启发式与 tool_use round-trip）| tags: llm, markdown, action-tag, ui-interaction, plugin, launcher, swiftui, button, tts, copy, prompt-engineering, protocol-design, structured-output | → patterns/2026-05-31-llm-embedded-action-tag-protocol.md
- [2026-05-31] launcher prompt-mode 必须剥离 keyword 前缀再传 LLM（长前缀优先 + 严格 word boundary 避 "trace" 被 "tr" 错切）| tags: launcher, prompt-mode, keyword-routing, llm, query-preprocessing, edge-case, plugin, translate, word-boundary, strict-matching | → patterns/2026-05-31-launcher-prompt-mode-strip-keyword-prefix.md
- [2026-05-31] systemPrompt 用 few-shot 真实示例驱动 markdown 输出，模板占位符（**WORD**/PHRASE）会被 LLM 字面照搬 | tags: llm, prompt-engineering, systemprompt, few-shot, template, placeholder, markdown, translate, output-format, in-context-learning, hallucination | → patterns/2026-05-31-llm-fewshot-vs-template-placeholders.md

## 2026-05-31 translate 性能彻查 (task 013) 新增条目
- [2026-05-31] Qwen3 thinking 关闭唯一通道是 `chat_template_kwargs.enable_thinking:false`（top-level / user `/no_think` 都被 llama-server 忽略；17× 加速，单条 24.5s→1.45s）| tags: qwen3, llm, thinking, reasoning, chat-template-kwargs, llama-cpp, openai-compat, perf, ttft, jinja, prompt-api | → patterns/2026-05-31-qwen3-thinking-disable-channel.md
- [2026-05-31] launcher router 短路：narrowCandidates 唯一/strong 命中（score≥10）跳过 aiSelect LLM call — AI 选 1 应当只在歧义存在时触发 | tags: launcher, router, ai-router, llm-call, perf, score, narrow-candidates, plugin-selection, architecture, yagni | → patterns/2026-05-31-launcher-router-shortcircuit-unique-match.md
- [2026-05-31] SwiftUI `AttributedString.MarkdownParsingOptions.inlineOnly*` 不消化 block markdown（`###`/`-`/`*` 显示字面）；预处理 `###`→`**bold**`、`-`/`*`→`•`、`---`→`─────` 解决，`.full` 模式 `Text` 视图也不会自动渲染块样式 | tags: swiftui, attributedstring, markdown, inline-only, block, heading, list, parsing-options, rendering, preprocess, launcher | → patterns/2026-05-31-swiftui-attributedstring-inline-only-block-markdown.md
- [2026-05-31] LLM systemPrompt 三层哲学：**风格定位词**（"X 助手"，激活已知 schema）> 形态约束词（"像字典"，不写数字）> negative 约束（"不写文化背景"），避免"对用户最有用 / 自由发挥"等扩展指令（触发教材 mode）和极简协议告知（触发"贴标签是主任务" mode）| tags: llm, prompt-engineering, systemprompt, style-positioning, role-prompt, anti-pattern, few-shot, action-protocol, qwen3, translate, dryrun | → patterns/2026-05-31-llm-prompt-style-positioning-vs-instructions.md
