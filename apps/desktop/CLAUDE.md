...
**SnipPanelVC 单列 accordion（autopilot 2026-07-13 重构）**：

旧 master-detail 双栏（左 leftPane 240 + 右 ContentColumnView 包 detailContainer 四态 empty/create/edit/preview）已废弃。根因：三重嵌套 ContentColumnView（gallery ContentColumnView → pluginPanelContainer → SnipPanelVC.view → 内层 ContentColumnView rightColumn → detailContainer），第二层 documentView 塌缩（patterns/2026-07-12 同构）+ `makePanelVC(){return self}` 违反新实例契约 → 右侧预览不显示。

新设计 = **单列全宽**（SnipPanelVC.view 铺满 gallery pluginPanelContainer）：
- **header**：NSSearchField + 「新增片段」button（水平 stack）
- **主体**：内层 NSScrollView 包 NSTableView（嵌套深度 2 = gallery ContentColumnView scrollView + snip 列表 scrollView；C-SNIP-SINGLE-COLUMN）
- **accordion**：每行折叠态卡片（keyword 主标题 + content 预览 + 行内 ✎/🗑 按钮）/ 展开态编辑表单（keyword + content NSTextView + 占位符提示 + 操作栏）。`expandedRow: Int?` 单值，同一时刻最多展开 1 项（C-SNIP-ACCORDION-ONE）；展开 A 自动折叠 B。
- **新建态**：`expandedRow = createRowIndex(-1)` 哨兵，tableView 顶部插入新建编辑行；保存成功转正常行。
- **makePanelVC**：`return SnipPanelVC()`（C-PANEL-NEW-INSTANCE，禁 return self）；register(SnipPanelVC(), for:"snip") 注册的实例仅作 provider 路由入口。
- **AX id**：折叠行 `settings.plugins.snip.row.<i>` / 展开行 `settings.plugins.snip.expanded.<i>`（i==-1 新建行）；既有 `settings.plugins.splitview` / `.detail` / `.sidebar.table` 不变（C-AX-STABLE）。
- **debug 字段**（`debugSettingsState`）：通用 `detail_content_height`（防白屏回归 C-CONTENTCOLUMN-NO-REGRESS）+ 选中 snip 时 `snip_expanded_visible` + `snip_expanded_height`。SnipPanelVC 暴露 `expandedRowIndex: Int?` + `expandedRowHeight: CGFloat` 只读访问器。
- **预备缓解**（BLOCKER B）：SnipPanelVC.view `autoresizingMask=[.width,.height]` + `viewDidLayout` 强制内层 listScrollView frame，防混合布局时序首帧塌缩（patterns/2026-07-12 同构盲区防回归）。
- **enum**：`SnipAccordionMode { empty / create / edit }`（testHook_currentDetailMode 语义；preview 态并入行内折叠卡片只读展示，不再单独 preview VC）。旧 `SnipDetailMode` 已删。

**AI 配置布局（autopilot 2026-07-13 优化，ProviderSettingsViewController）**：

旧「关闭思考」并入模型行（modelControlStack = [modelField | noThinking label+switch]）+ 测试连接并入 API 地址行（urlControlStack = [baseURLField | testButton | spinner]）水平挤压，已重构：
- **关闭思考**：拆出独立 `SettingsToggleRow`（title「关闭思考」+ subtitle「适用于 Qwen3 等推理模型」+ SageSwitch），仅 openai-compatible 显示（`noThinkingRow?.isHidden` 切换，从旧 `noThinkingContainer.isHidden` 迁移）。插在 modelRow 与 baseURLRow 之间。modelRow control 还原单 modelField（subtitle 改「留空则使用提供者默认模型」）。
- **连接测试**：拆出独立行（`SettingsFormRow` title「连接测试」+ subtitle + 水平 stack[testButton + spinner]）。结果文案仍用 testResultRow（视觉属同一动作反馈组，紧邻）。baseURLRow control 还原单 baseURLField。插在 baseURLRow 与 apiKeyRow 之间（替代旧 testResultRow 位置）。
- **契约 C-AI-ONE-CONTROL-PER-ROW**：每行单一主输入 control（model 行=modelField；baseURL 行=baseURLField；关闭思考=独立 toggle row；连接测试=独立 action row，其 button+spinner 属同一动作反馈允许水平排列）。不同 SettingsFormRow 行容器 Y 轴不重叠。
...
- ⚠️ **NSSplitViewController 缩窗 + ContentColumnView documentView 塌缩双坑**（2026-07-12 真机实测）：
  - **缩窗坑**：`contentViewController` 是 `NSSplitViewController` 时，它按 splitView 的 fittingSize 缩窗（bypass `setContentSize`/`minSize`），窗口被钉在 fittingSize（sidebar 200 + detail ContentColumnView fittingWidth 600 ≈ 808×572）。修法（`SettingsSplitViewController.viewDidLoad`）：给 splitView 加 `view.widthAnchor ≥ visibleFrame.width` + `view.heightAnchor ≥ visibleFrame.height` 抬高 fittingSize → 缩窗目标 = visibleFrame → 窗口充满屏幕（真机 1920×1082）。`visibleFrame = NSScreen.main?.visibleFrame ?? NSSize(1920,1080)` 兜底 headless。`SettingsWindowController.computeInitialSize` 同步 0.75→1.0。
  - **ContentColumnView documentView 塌缩坑（白屏真根因）**：ContentColumnView 的 documentView 嵌套 NSSplitViewController 下 autolayout 约束钉尺寸**全部失效**（width/top/height，钉 contentView/scrollView 均无效，NSScrollView tile 覆盖 autolayout）→ documentView 0×0 → contentColumn 0 高 → 右侧整片白屏（plugins/hotkey/ai/general/about 5 section；skins 不用 ContentColumnView 故幸免）。修法（`ContentColumnView`）：documentView `autoresizingMask = [.width]` + 删全部 autolayout 约束 + `layout()` override 手动设 `documentView.frame = (0,0,clipWidth,scrollView.bounds.height)`。详见 `.autopilot/knowledge/patterns/2026-07-12-nsscrollview-documentview-autolayout-nested-splitview-collapse-manual-frame.md`。
  - **诊断**：NSScrollView layout 问题用 **in-process bounds**（`BuddyLogger` dump detail 链 documentView/clipView bounds，`buddy launcher debug get-state` + `buddy log grep` 读）诊断，**截图裁剪不可靠**（窗口 `center()` 随 section 宽移位 + Cocoa y 左下角换算错裁到空白，曾致多轮误判「已修复」）。截图像素方差只判「窗口某处有内容」，不能定位「哪个 view 0×0」。已知 trade-off：documentView 高度固定 = scrollView 高，超高内容截断不滚动（大窗口下设置内容不超高 + ProviderSettings 有内层 jsonScrollView 兜底）。
...
QA 阶段不能只跑单元测试就写结论，必须启动真实 app 做端到端验证。**尤其 GUI / 布局 / 窗口 sizing / NSHostingController / 动画类变更**：headless `swift test`（无完整 window server session）对这些有盲区——例如 hosting controller 的 `preferredContentSize` 经 VC 链传到顶层 window、NSScrollView documentView 贴底对齐等行为在测试环境复现不了（patterns/2026-07-09 / 2026-07-03）。这类改动单元测试只能守「根因属性」（如 sizingOptions 值、fittingSize），端到端效果必须真机确认。

**默认流程**（改了 desktop app 就走一遍，别在 headless 单测绿了就下「修好」结论；也避免反复让用户手动重启）：

```bash
# 1. 构建（SKIP_FETCH_PLUGINS=1 跳过 fetch 加速，已有 plugins 内容时）
SKIP_FETCH_PLUGINS=1 make bundle

# 2. 关旧 + 启新（一条命令；旧 app PID 也可 pgrep 查后 kill）
pkill -f ClaudeCodeBuddy; sleep 1; open apps/desktop/ClaudeCodeBuddy.app

# 3. 猫咪状态类：buddy CLI 逐场景验证
buddy session start --id debug-A --cwd ~/myproject
buddy inspect --id debug-A          # JSON 证据写入 QA 报告
buddy session end --id debug-A
```
