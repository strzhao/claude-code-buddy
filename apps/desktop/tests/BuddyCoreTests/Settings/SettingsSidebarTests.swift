...
//
// 覆盖可自动化验证的契约：
//   - 契约 3：6 项顺序 [插件/热键/AI 配置/皮肤/通用/关于]（皮肤紧贴通用）
//   - 契约 2：allCases 单一数据源
...
final class SettingsSidebarTests: XCTestCase {
    ...
    func test_section_order_is_plugins_hotkey_ai_skins_general_about() {
        let rawValues = SettingsSection.allCases.map { $0.rawValue }
        XCTAssertEqual(rawValues, ["plugins", "hotkey", "ai", "skins", "general", "about"],
                       "sidebar 顺序必须为 [插件/热键/AI 配置/皮肤/通用/关于]（皮肤紧贴通用）")
    }
