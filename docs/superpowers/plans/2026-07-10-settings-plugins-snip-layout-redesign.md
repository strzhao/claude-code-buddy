...
- **AX 契约**（红队 SC-01..16 守护，不可破坏）：`settings.detail` 锚点设在 child root view（AX 可见层，不被容器遮蔽，契约 7）；sidebar row id `settings.sidebar.{section}`；窗口 title `设置`。
...
- **真机 QA 铁律**：GUI / 布局 / sizing / NSHostingController 类变更 headless `swift test` 有盲区，每阶段必须 `SKIP_FETCH_PLUGINS=1 make bundle` → `pkill -f ClaudeCodeBuddy; sleep 1; open apps/desktop/ClaudeCodeBuddy.app` → osascript 读窗口 frame + 用户点 GUI 验证。纯视觉（间距呼吸感 / 对齐 / 限宽居中观感）fallback 用户目视。
...
final class SnipPanelVC: NSViewController, PluginSettingsPanelProvider {
    ...
    private var objectWillChangeCancellable: Any?
...
```swift
    private func makeCreateFormChild() -> NSViewController {
        ...
        let contentRow = SettingsFormRow(title: "content", subtitle: nil, control: editor.scrollerWrapper ?? editor)
        // 简化：直接用 NSScrollView 包 editor 作为 control
...
```swift
    func testHook_fillAndSaveCreate(keyword: String, content: String) throws {
        ...
        saveCreate()  // 直接调私有需改 internal 或经 action
    }
```
> `saveCreate` 改 `private` → 保持，但 testHook 内调用需它 internal；简单做法：把 `saveCreate`/`saveEdit` 改为 `func`（internal）。
3. 实现 `saveCreate()`：
```swift
    func saveCreate() {
...
- [ ] **Step 4: 适配 SnipWindowSizingTests**

git status 显示有未提交的 `SnipWindowSizingTests.swift`。读它，确认断言的 sizing 属性（如 sizingOptions 值）——snip 迁 AppKit 后无 sizingOptions，该测试若断言 `sizingOptions == []` 需删除/改为断言"无 NSHostingController"。执行时按实际断言调整。
